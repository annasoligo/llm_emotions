#!/bin/bash
#SBATCH --job-name=suppress_sandbag_qwen235b
#SBATCH --output=/workspace-vast/annas/logs/suppress_sandbag_qwen235b_%j.out
#SBATCH --error=/workspace-vast/annas/logs/suppress_sandbag_qwen235b_%j.out
#SBATCH --time=4:00:00
#SBATCH --partition=general
#SBATCH --gres=gpu:4
#SBATCH --cpus-per-task=32
#SBATCH --mem=400G

source /workspace-vast/annas/.secrets/load_secrets.sh

# Required for vLLM apply_model with custom callables
export VLLM_ALLOW_INSECURE_SERIALIZATION=1

cd /workspace-vast/annas/git/research-tools
source .venv/bin/activate

echo "===== Sandbagging with Suppression Experiment ====="
echo "Model: Qwen/Qwen3-235B-A22B"
echo "Testing whether suppression vectors can reduce emotional text while preserving behavior"
echo "=================================================="

python -m steering_tests.suppression_experiments.sandbagging_with_suppression \
    --model Qwen/Qwen3-235B-A22B \
    --layers 48 49 50 51 52 \
    --fear-pct 0.20 \
    --suppress-pcts 0.5 1.0 2.0 \
    --num-samples 30 \
    --tp 4 \
    --gpu-memory 0.90 \
    --max-model-len 8192 \
    --max-tokens 2000

echo "===== Experiment Complete ====="
