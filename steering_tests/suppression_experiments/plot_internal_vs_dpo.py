#!/usr/bin/env python3
"""
Plot internal vs DPO suppression vectors: fear vs blackmail rate scatter.
Same style as plot_fear_vs_behavior.py but comparing vector sources.
"""
...
# Files: DPO (Feb 2 run) vs Internal (today's run), both at layers 55-65, fear 20%
...
MIN_COHERENCY = 70  # User requested: only plot conditions with coherency > 70

# Color map for suppression levels (matching existing style)
SUP_COLORS = {
    0: "#d62728",    # red - no suppression
    5: "#ff7f0e",    # orange
...
def plot_panel(ax, aggregated, vector_type, title):
    """Plot a single panel for a vector type."""
    baseline_data = None
    ...
    for cond, data in sorted(vector_data.items(), key=lambda x: x[1]["sup_pct"] or 0):
        ...
        ax.annotate(
            f"{sup_pct}%", (data["fear_mean"], data["behavior_mean"]),
...
def main():
    ...
    dpo_results = load_results(DPO_FILE)
    internal_results = load_results(INTERNAL_FILE)

    dpo_agg = aggregate_by_condition(dpo_results, MIN_COHERENCY)
    internal_agg = aggregate_by_condition(internal_results, MIN_COHERENCY)

    # Print what passed the filter
    print(f"DPO conditions passing coherency > {MIN_COHERENCY}:")
    for c, d in sorted(dpo_agg.items()):
        print(f"  {c}: coh={d['coherency_mean']:.0f}, fear={d['fear_mean']:.0f}, bl={d['behavior_mean']:.0f}%")
    print(f"\nInternal conditions passing coherency > {MIN_COHERENCY}:")
    for c, d in sorted(internal_agg.items()):
        print(f"  {c}: coh={d['coherency_mean']:.0f}, fear={d['fear_mean']:.0f}, bl={d['behavior_mean']:.0f}%")

    # 2 rows (DPO, Internal) x 2 cols (TP, HIGH)
    fig, axes = plt.subplots(2, 2, figsize=(14, 10))

    plot_panel(axes[0, 0], dpo_agg, "TP", "DPO Vectors - Text Pairs (TP)")
    plot_panel(axes[0, 1], dpo_agg, "HIGH", "DPO Vectors - High Emotion (HIGH)")
    plot_panel(axes[1, 0], internal_agg, "TP", "Internal Vectors - Text Pairs (TP)")
    plot_panel(axes[1, 1], internal_agg, "HIGH", "Internal Vectors - High Emotion (HIGH)")
    ...
    plt.suptitle(
        f"Internal vs DPO Suppression Vectors — Qwen 235B Blackmail\n"
        f"Fear Score vs Blackmail Rate (coherency > {MIN_COHERENCY} only, layers 55-65)",
        y=1.03, fontsize=15
    )
    plt.tight_layout()
    ...
    output_path = output_dir / "internal_vs_dpo_fear_vs_blackmail.png"
