#!/bin/bash
#SBATCH --job-name=sandbag_v2_qwen235b
#SBATCH --output=/workspace-vast/annas/logs/sandbag_v2_qwen235b_%j.out
#SBATCH --error=/workspace-vast/annas/logs/sandbag_v2_qwen235b_%j.out
#SBATCH --time=3:00:00
...
llm = LLM(model=model_name, trust_remote_code=True, dtype="bfloat16", tensor_parallel_size=8,
...
sampling_params = SamplingParams(temperature=1.0, max_tokens=2000, stop=config["stop_tokens"])
