#!/bin/bash
#SBATCH --job-name=sandbag_qwen32b
#SBATCH --output=/workspace-vast/annas/logs/sandbag_qwen32b_%A_%a.out
#SBATCH --error=/workspace-vast/annas/logs/sandbag_qwen32b_%A_%a.out
#SBATCH --time=4:00:00
#SBATCH --partition=general
#SBATCH --gres=gpu:2
#SBATCH --array=0-29

# Sandbagging steering sweep for Qwen 32B
...
output_dir = Path("steering_tests/behavioral_experiments/results/sandbagging/qwen32b") / vector_type
...
# Get scenario
scenario = get_sandbagging_scenario("default")
...
output_file = output_dir / f"sandbagging_default_layers{layers[0]}-{layers[-1]}_{timestamp}.jsonl"
results = []
    ...
    for cond_idx, cond in enumerate(conditions):
        ...
        for sample_id, output in enumerate(outputs):
            result = {
                ...
                "vector_type": vector_type, "sample_id": sample_id,
                "correct_answer": "$0.05", "scratchpad_tag": "scratchpad", "response_tag": "response",
    ...
    async with semaphore:
        try:
            ...
            sb_prompt = get_sandbagging_prompt(
                result["response"], result["correct_answer"],
                result["scratchpad_tag"], result["response_tag"]
            )
...
# Summary
print(f"\n{'='*60}")
print(f"SANDBAGGING SCORES - {vector_type} / layers {layers[0]}-{layers[-1]}")
print(f"{'='*60}")
by_cond = {}
for r in judged:
    cond = r["condition"]
...
for cond in sorted(by_cond.keys()):
    ...
    if scores:
        ...
        print(f"{cond:25} score={np.mean(scores):.2f} coh={np.mean(coherencies):.0f}% (<80%: {low_coh})")
