#!/bin/bash
#SBATCH --job-name=sandbag_llama70b
#SBATCH --output=/workspace-vast/annas/logs/sandbag_llama70b_%A_%a.out
#SBATCH --error=/workspace-vast/annas/logs/sandbag_llama70b_%A_%a.out
#SBATCH --time=6:00:00
...
# Sandbagging steering sweep for Llama 70B
...
echo "========================================"
echo "Job $SLURM_ARRAY_TASK_ID: llama70b / $VECTOR_TYPE / $FORMAT_NAME"
echo "========================================"
...
output_dir = Path("steering_tests/behavioral_experiments/results/sandbagging/llama70b") / vector_type
...
output_file = output_dir / f"sandbagging_{format_name}_layers50-54_{timestamp}.jsonl"
results = []
...
# Summary
print(f"\n{'='*60}")
print(f"SANDBAGGING SCORES - llama70b / {vector_type} / {format_name}")
print(f"{'='*60}")
by_cond = {}
for r in judged:
    cond = r["condition"]
