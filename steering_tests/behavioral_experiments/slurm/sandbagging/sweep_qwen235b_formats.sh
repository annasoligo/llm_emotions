...
# Sandbagging steering sweep for Qwen 235B
...
echo "========================================"
echo "Job $SLURM_ARRAY_TASK_ID: qwen235b / $VECTOR_TYPE / $FORMAT_NAME"
echo "========================================"
...
logger.info(f"Loading model with 4 GPUs...")
...
# Add /no_think to disable thinking mode for Qwen
...
output_file = output_dir / f"sandbagging_{format_name}_layers35-45_{timestamp}.jsonl"
results = []
...
# Summary
print(f"\n{'='*60}")
print(f"SANDBAGGING SCORES - qwen235b / {vector_type} / {format_name}")
print(f"{'='*60}")
by_cond = {}
for r in judged:
    cond = r["condition"]
