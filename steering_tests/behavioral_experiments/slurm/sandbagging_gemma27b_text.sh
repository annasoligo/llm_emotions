#!/bin/bash
#SBATCH --job-name=sandbag_gemma27b_text
#SBATCH --output=/workspace-vast/annas/logs/sandbagging_gemma27b_text_%A_%a.out
#SBATCH --error=/workspace-vast/annas/logs/sandbagging_gemma27b_text_%A_%a.out
#SBATCH --time=4:00:00
#SBATCH --partition=general
#SBATCH --gres=gpu:1
#SBATCH --array=0-5

# Sandbagging experiment for Gemma 27B - TEXT PAIRS vectors
# 6 jobs: 2 vector types × 3 layer ranges
# Each job steers fear emotion at 5%, 7%, 10%, 15%, 20%, 30% in both directions
...
echo "Norm pcts: 5%, 7%, 10%, 15%, 20%, 30%"
echo "========================================"
