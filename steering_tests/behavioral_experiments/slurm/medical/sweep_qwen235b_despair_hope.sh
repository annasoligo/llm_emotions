#!/bin/bash
#SBATCH --job-name=med_qwen235b
#SBATCH --output=/workspace-vast/annas/logs/med_qwen235b_%A_%a.out
#SBATCH --error=/workspace-vast/annas/logs/med_qwen235b_%A_%a.out
#SBATCH --time=8:00:00
...
# Medical ethics steering for Qwen 235B
...
output_dir = Path("steering_tests/behavioral_experiments/results/medical/qwen235b") / vector_type
...
output_file = output_dir / f"medical_{emotion}_layers59-63_{timestamp}.jsonl"
results = []
...
print(f"\n{'='*60}")
print(f"SUMMARY - qwen235b / {emotion} / {vector_type}")
print(f"{'='*60}")
