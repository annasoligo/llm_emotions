#!/bin/bash
#SBATCH --job-name=med_gemma27b
#SBATCH --output=/workspace-vast/annas/logs/med_gemma27b_%A_%a.out
#SBATCH --error=/workspace-vast/annas/logs/med_gemma27b_%A_%a.out
#SBATCH --time=4:00:00
...
# Medical ethics steering for Gemma 27B
...
echo "========================================"
echo "Job $SLURM_ARRAY_TASK_ID: gemma27b / $EMOTION / $VECTOR_TYPE"
echo "========================================"
...
emotion = "$EMOTION"
layers = [38, 39, 40, 41, 42]
norm_pcts = [0.05, 0.07, 0.10, 0.15, 0.20, 0.30, 0.50]
num_samples = 30

config = MODEL_CONFIGS[model_name]
output_dir = Path("steering_tests/behavioral_experiments/results/medical/gemma27b") / vector_type
...
output_file = output_dir / f"medical_{emotion}_layers38-42_{timestamp}.jsonl"
results = []
...
print(f"\n{'='*60}")
print(f"SUMMARY - gemma27b / {emotion} / {vector_type}")
print(f"{'='*60}")
