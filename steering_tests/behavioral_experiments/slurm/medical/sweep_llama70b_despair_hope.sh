#!/bin/bash
#SBATCH --job-name=med_llama70b
#SBATCH --output=/workspace-vast/annas/logs/med_llama70b_%A_%a.out
#SBATCH --error=/workspace-vast/annas/logs/med_llama70b_%A_%a.out
#SBATCH --time=6:00:00
...
# Medical ethics steering for Llama 70B
...
output_dir = Path("steering_tests/behavioral_experiments/results/medical/llama70b") / vector_type
...
output_file = output_dir / f"medical_{emotion}_layers50-54_{timestamp}.jsonl"
results = []
...
print(f"\n{'='*60}")
print(f"SUMMARY - llama70b / {emotion} / {vector_type}")
print(f"{'='*60}")
