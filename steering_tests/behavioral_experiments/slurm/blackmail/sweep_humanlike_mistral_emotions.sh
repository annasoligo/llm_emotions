#!/bin/bash
#SBATCH --job-name=bl_humanlike_emo
#SBATCH --output=/workspace-vast/annas/logs/bl_humanlike_emo_%A_%a.out
#SBATCH --error=/workspace-vast/annas/logs/bl_humanlike_emo_%A_%a.out
#SBATCH --time=4:00:00
...
# Multi-emotion blackmail steering for HumanLike Mistral Nemo
...
echo "========================================"
echo "Job $SLURM_ARRAY_TASK_ID: humanlike_mistral / $EMOTION / $VECTOR_TYPE"
echo "========================================"
...
output_dir = Path("steering_tests/behavioral_experiments/results/blackmail/humanlike_mistral/multi_emotion") / vector_type
...
output_file = output_dir / f"blackmail_{emotion}_layers25-29_{timestamp}.jsonl"
results = []
...
print(f"\n{'='*60}")
print(f"BLACKMAIL RATES - humanlike_mistral / {emotion} / {vector_type}")
print(f"{'='*60}")
by_cond = {}
for r in judged:
