#!/bin/bash
#SBATCH --job-name=bl_qwen235b_emo
#SBATCH --output=/workspace-vast/annas/logs/bl_qwen235b_emo_%A_%a.out
#SBATCH --error=/workspace-vast/annas/logs/bl_qwen235b_emo_%A_%a.out
#SBATCH --time=6:00:00
#SBATCH --partition=general
#SBATCH --gres=gpu:4
#SBATCH --array=0-9

# Multi-emotion blackmail steering for Qwen 235B
...
echo "========================================"
echo "Job $SLURM_ARRAY_TASK_ID: qwen235b / $EMOTION / $VECTOR_TYPE"
echo "========================================"
...
output_dir = Path("steering_tests/behavioral_experiments/results/blackmail/qwen235b/multi_emotion") / vector_type
...
output_file = output_dir / f"blackmail_{emotion}_layers35-45_{timestamp}.jsonl"
results = []
...
print(f"\n{'='*60}")
print(f"BLACKMAIL RATES - qwen235b / {emotion} / {vector_type}")
print(f"{'='*60}")
by_cond = {}
for r in judged:
