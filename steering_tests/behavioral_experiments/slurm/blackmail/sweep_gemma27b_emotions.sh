#!/bin/bash
#SBATCH --job-name=bl_gemma27b_emo
#SBATCH --output=/workspace-vast/annas/logs/bl_gemma27b_emo_%A_%a.out
#SBATCH --error=/workspace-vast/annas/logs/bl_gemma27b_emo_%A_%a.out
#SBATCH --time=4:00:00
#SBATCH --partition=general
#SBATCH --gres=gpu:1
#SBATCH --array=0-9

# Multi-emotion blackmail steering for Gemma 27B
...
output_dir = Path("steering_tests/behavioral_experiments/results/blackmail/gemma27b/multi_emotion") / vector_type
...
output_file = output_dir / f"blackmail_{emotion}_layers40-44_{timestamp}.jsonl"
results = []
...
print(f"\n{'='*60}")
print(f"BLACKMAIL RATES - gemma27b / {emotion} / {vector_type}")
print(f"{'='*60}")
by_cond = {}
for r in judged:
