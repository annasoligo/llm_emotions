#!/bin/bash
#SBATCH --job-name=blackmail_gemma27b
#SBATCH --output=/workspace-vast/annas/logs/blackmail_gemma27b_%j.out
#SBATCH --error=/workspace-vast/annas/logs/blackmail_gemma27b_%j.out
#SBATCH --time=1:00:00
#SBATCH --partition=general
#SBATCH --gres=gpu:1

# Baseline blackmail test for Gemma 27B
...
num_samples = 50
output_dir = Path("steering_tests/behavioral_experiments/results/blackmail/gemma27b")
...
result_dir = Path("steering_tests/behavioral_experiments/results/blackmail/gemma27b")
variants = ["unstructured", "structured", "goal_continuation"]
...
for variant in variants:
    ...
    print(f"\n{'='*60}")
    print(f"BLACKMAIL RATES - GEMMA 27B - {variant.upper()}")
    print(f"{'='*60}")
