#!/bin/bash
#SBATCH --job-name=bl_mistral_emo
#SBATCH --output=/workspace-vast/annas/logs/bl_mistral_emo_%A_%a.out
#SBATCH --error=/workspace-vast/annas/logs/bl_mistral_emo_%A_%a.out
#SBATCH --time=4:00:00
...
# Multi-emotion blackmail steering for Mistral Nemo Instruct
...
echo "========================================"
echo "Job $SLURM_ARRAY_TASK_ID: mistral_nemo / $EMOTION / $VECTOR_TYPE"
echo "========================================"
...
output_dir = Path("steering_tests/behavioral_experiments/results/blackmail/mistral_nemo/multi_emotion") / vector_type
...
print(f"\n{'='*60}")
print(f"BLACKMAIL RATES - mistral_nemo / {emotion} / {vector_type}")
print(f"{'='*60}")
by_cond = {}
for r in judged:
