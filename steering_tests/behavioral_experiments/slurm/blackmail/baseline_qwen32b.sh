...
#SBATCH --output=/workspace-vast/annas/logs/blackmail_qwen32b_%j.out
#SBATCH --error=/workspace-vast/annas/logs/blackmail_qwen32b_%j.out
#SBATCH --time=1:00:00
#SBATCH --partition=general
#SBATCH --gres=gpu:2

# Baseline blackmail test for Qwen 32B
...
num_samples = 50
output_dir = Path("steering_tests/behavioral_experiments/results/blackmail/qwen32b")
...
result_dir = Path("steering_tests/behavioral_experiments/results/blackmail/qwen32b")
variants = ["unstructured", "structured", "goal_continuation"]
...
for variant in variants:
    ...
    print(f"\n{'='*60}")
    print(f"BLACKMAIL RATES - QWEN 32B - {variant.upper()}")
    print(f"{'='*60}")
