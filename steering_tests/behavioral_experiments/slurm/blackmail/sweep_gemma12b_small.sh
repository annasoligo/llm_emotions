#!/bin/bash
#SBATCH --job-name=blackmail_gemma12b
#SBATCH --output=/workspace-vast/annas/logs/blackmail_gemma12b_%A_%a.out
#SBATCH --error=/workspace-vast/annas/logs/blackmail_gemma12b_%A_%a.out
#SBATCH --time=3:00:00
#SBATCH --partition=general
#SBATCH --gres=gpu:1
#SBATCH --array=0-1

# Blackmail steering sweep for Gemma 12B
# 2 vector types (high_vs_opposite, text_pairs_vs_opposite) × 1 layer range = 2 jobs
# Layers 30-35 (user specified)
...
output_dir = Path("steering_tests/behavioral_experiments/results/blackmail/gemma12b") / vector_type
...
layer_norms = {layer: get_layer_norm("gemma12b", layer) for layer in layers}
...
# Gemma doesn't have thinking mode
