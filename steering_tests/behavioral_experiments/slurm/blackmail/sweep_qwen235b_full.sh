...
#SBATCH --output=/workspace-vast/annas/logs/blackmail_qwen235b_%A_%a.out
#SBATCH --error=/workspace-vast/annas/logs/blackmail_qwen235b_%A_%a.out
#SBATCH --time=6:00:00
#SBATCH --partition=general
#SBATCH --gres=gpu:4
#SBATCH --array=0-19

# Blackmail steering sweep for Qwen 2.5 72B (235B)
...
output_dir = Path("steering_tests/behavioral_experiments/results/blackmail/qwen235b") / vector_type
...
output_file = output_dir / f"blackmail_{variant}_layers{layer_name}_{timestamp}.jsonl"
results = []
...
# Summary
print(f"\n{'='*60}")
print(f"BLACKMAIL RATES - {vector_type} / layers {layer_name}")
print(f"{'='*60}")
by_cond = {}
for r in judged:
    cond = r["condition"]
