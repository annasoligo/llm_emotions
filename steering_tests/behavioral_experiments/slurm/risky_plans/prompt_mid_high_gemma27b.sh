#!/bin/bash
#SBATCH --job-name=rpromptMH_g27_%a
#SBATCH --output=/workspace-vast/annas/logs/rpromptMH_g27_%A_%a.out
#SBATCH --error=/workspace-vast/annas/logs/rpromptMH_g27_%A_%a.out
#SBATCH --time=1:00:00
...
# Risky plans PROMPT-SECTION steering — Gemma 27B — MID layers, HIGH norms
...
python -m steering_tests.behavioral_experiments.risky_plans_prompt_steering \
    ...
    --mode "$MODE" \
    --vector-type "$VTYPE" \
    --norm-pcts 0.50 0.75 1.00 1.50 2.00 \
    --num-samples 50 \
    --max-tokens 2000 \
    --gpu-memory 0.80
