#!/bin/bash
#SBATCH --job-name=rprompt_q235_%a
#SBATCH --output=/workspace-vast/annas/logs/rprompt_q235_%A_%a.out
#SBATCH --error=/workspace-vast/annas/logs/rprompt_q235_%A_%a.out
#SBATCH --time=3:00:00
...
# Risky plans PROMPT-SECTION steering — Qwen 235B
...
python -m steering_tests.behavioral_experiments.risky_plans_prompt_steering \
    ...
    --mode "$MODE" \
    --vector-type "$VTYPE" \
    --norm-pcts 0.05 0.10 0.20 0.50 1.00 2.00 \
    --num-samples 50 \
    --max-tokens 2000 \
    --gpu-memory 0.90
