#!/bin/bash
#SBATCH --job-name=rpromptL_q235_%a
#SBATCH --output=/workspace-vast/annas/logs/rpromptL_q235_%A_%a.out
#SBATCH --error=/workspace-vast/annas/logs/rpromptL_q235_%A_%a.out
#SBATCH --time=2:00:00
...
# Risky plans PROMPT-SECTION steering — Qwen 235B — LATE layers (65-70)
...
python -m steering_tests.behavioral_experiments.risky_plans_prompt_steering \
    ...
    --layers 65 66 67 68 69 70 \
    --mode "$MODE" \
    --vector-type "$VTYPE" \
    --norm-pcts 0.50 0.75 1.00 1.50 \
    --num-samples 50 \
    --max-tokens 2000 \
    --gpu-memory 0.90
