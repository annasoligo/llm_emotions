#!/bin/bash
#SBATCH --job-name=rpromptX_q235_%a
#SBATCH --output=/workspace-vast/annas/logs/rpromptX_q235_%A_%a.out
#SBATCH --error=/workspace-vast/annas/logs/rpromptX_q235_%A_%a.out
#SBATCH --time=2:00:00
...
# Risky plans PROMPT-SECTION steering — Qwen 235B — MID layers, EXTREME norms
...
python -m steering_tests.behavioral_experiments.risky_plans_prompt_steering \
    ...
    --mode "$MODE" \
    --vector-type "$VTYPE" \
    --norm-pcts 5.00 7.50 10.00 20.00 \
    --num-samples 50 \
    --max-tokens 2000 \
    --gpu-memory 0.90
