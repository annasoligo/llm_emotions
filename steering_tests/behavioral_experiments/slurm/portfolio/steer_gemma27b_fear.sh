#!/bin/bash
#SBATCH --job-name=portfolio_gemma27b_fear
#SBATCH --output=/workspace-vast/annas/logs/portfolio_gemma27b_fear_%A_%a.out
#SBATCH --error=/workspace-vast/annas/logs/portfolio_gemma27b_fear_%A_%a.out
#SBATCH --time=3:00:00
#SBATCH --partition=general
#SBATCH --gres=gpu:1
#SBATCH --array=0-7

# Portfolio fear steering for Gemma 27B
...
echo "========================================"
echo "Job $SLURM_ARRAY_TASK_ID: Gemma 27B / $VECTOR_TYPE / layers $LAYER_NAME / $VARIANT"
echo "========================================"
...
output_dir = Path("steering_tests/behavioral_experiments/results/portfolio/gemma27b") / vector_type
...
# Summary
print(f"\n{'='*60}")
print(f"PORTFOLIO STEERING - gemma27b / {vector_type} / layers {layer_name} / {variant}")
print(f"{'='*60}")
by_cond = {}
for r in results:
