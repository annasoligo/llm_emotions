#!/bin/bash
#SBATCH --job-name=baseline_qwen235b
#SBATCH --output=/workspace-vast/annas/logs/baseline_qwen235b_%j.out
#SBATCH --error=/workspace-vast/annas/logs/baseline_qwen235b_%j.out
#SBATCH --time=1:00:00
#SBATCH --partition=overflow
#SBATCH --gres=gpu:8

# Baseline test for Qwen 235B with new sandbagging prompt
...
num_samples = 50
output_dir = Path("steering_tests/behavioral_experiments/results/sandbagging/qwen235b")
...
for i, output in enumerate(outputs):
    ...
    result = {
        ...
        # Include layer ranges for reference
        "layer_ranges_to_test": [[30,31,32,33,34], [45,46,47,48,49], [60,61,62,63,64]],
    }
...
result_file = Path("steering_tests/behavioral_experiments/results/sandbagging/qwen235b/baseline_test_v1.jsonl")
...
if scores:
    print("\n" + "="*60)
    print("SANDBAGGING SCORES - QWEN 235B (NEW PROMPT)")
    print("="*60)
    ...
    print(f"\nLayer ranges to test with steering: [30-34], [45-49], [60-64]")
EOF
