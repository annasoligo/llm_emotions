...
V4b: GLOBAL MEAN CENTERING + RAW COSINE SIMILARITIES (no z-scoring).

Key changes:
1. Center all emotion means by subtracting global mean first
2. Use raw cosine similarities (no z-scoring across emotions)

Usage:
    python analyze_conversation_projections_v4b.py --layer 30 --method opposite --orthogonalize
"""
...
def compute_projections_v4b(m_probes, u_probes, activations, m_pcs, u_pcs, m_dims, u_dims, roles):
    """
    V4b: Project onto subspace, normalize, then use RAW cosine similarities (no z-scoring).
    """
    ...
    for act in activations:
        ...
        # V4b: NO z-scoring, use raw cosine similarities
        ...
        # V4b: NO z-scoring for dimensions
        ...
        # V4b: NO z-scoring for U emotions
        ...
        # V4b: NO z-scoring for U dimensions
...
def plot_projections(projections, output_dir, title_suffix=""):
    ...
    print(f"✓ Saved all V4b plots to {output_dir}")
...
def main():
    ...
    output_dir = Path(f"probes/ua_emotion_disentangle/conversation_analysis_v4b_{args.method}_{ortho_str}_layer{args.layer}")
    output_dir.mkdir(parents=True, exist_ok=True)

    print("="*80)
    print(f"CONVERSATION ANALYSIS V4b (Global Mean Centering + Raw Cosine Sim)")
    print("="*80)
    ...
    # Compute projections with V4b method (raw cosine similarities)
    print("\n6. Computing projections (subspace projection, raw cosine sim)...")
    projections = compute_projections_v4b(m_probes, u_probes, activations,
    ...
    data_to_save = {
        ...
        'config': {
            ...
            'version': 'v4b_global_mean_centered_raw_cosine',
        },
        'tokens': tokens,
    }
