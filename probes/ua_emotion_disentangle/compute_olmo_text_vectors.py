#!/usr/bin/env python3
"""Compute mean-difference steering vectors from OLMo text activations.

Creates steering vectors by computing:
    vector[emotion] = mean(emotional_activations) - mean(neutral_activations)

Usage:
    python compute_olmo_text_vectors.py --layer 30 --output-dir vectors
"""
...
def load_text_activations(
    ...
    Args:
        h5_path: Path to OLMo texts h5 file
...
def save_vectors(
    ...
    for emotion, vector in vectors.items():
        output_path = output_dir / f"{emotion}_olmo_textmeandiff_layer{layer}.npz"
        np.savez(
            output_path,
            vector=vector,
            layer=layer,
            emotion=emotion,
            source="olmo_text_mean_diff",
            normalized=True,
    ...
    all_path = output_dir / f"all_emotions_olmo_textmeandiff_layer{layer}.npz"
    np.savez(
        all_path,
        vectors=all_vectors,
...
def main():
    ...
    parser.add_argument(
        "--data",
        type=str,
        default="/workspace-vast/annas/git/research-tools/probes/ua_emotion_disentangle/data/olmo_32b_texts_combined.h5",
        help="Path to OLMo text activations HDF5",
    )
    ...
    print("=" * 60)
    print("OLMO TEXT MEAN-DIFF VECTOR COMPUTATION")
    print("=" * 60)
    ...
    # Compute baseline std and L2 norm mean
    all_acts = []
    for emotion, (emotional_acts, neutral_acts) in emotion_data.items():
        all_acts.append(emotional_acts)
        all_acts.append(neutral_acts)
    combined = np.vstack(all_acts)
    baseline_std = float(np.std(combined, axis=0).mean())
    l2_norm_mean = float(np.linalg.norm(combined, axis=1).mean())

    print(f"\nBaseline statistics:")
    print(f"  Per-dim std (mean): {baseline_std:.2f}")
    print(f"  L2 norm (mean): {l2_norm_mean:.2f}")
    ...
    save_vectors(vectors, stds, args.layer, args.output_dir, baseline_std, l2_norm_mean)
    ...
    for emotion in EMOTIONS:
        if emotion in stds:
            print(f'    "{emotion}_olmo_textmeandiff": {stds[emotion]:.2f},')
