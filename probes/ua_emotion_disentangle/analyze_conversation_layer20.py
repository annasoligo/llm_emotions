...
def compute_projections(
    ...
    for i, act in enumerate(activations):
        ...
        # Project onto M PCs (using M-normalized activation)
        for j in range(4):
            proj = np.dot(m_normalized, m_pcs_norm[j])
            results[f'M_pc{j+1}'].append(proj)

        # Project onto U PCs (using U-normalized activation)
        for j in range(4):
...
def main():
    ...
    probes_file = Path("probes/ua_emotion_disentangle/orthogonal_probes_all_layers/layer_20_orthogonal.npz")
    output_dir = Path("probes/ua_emotion_disentangle/conversation_analysis_layer20")
    ...
    # Load orthogonalized probes
    print("\n3. Loading orthogonalized probes...")
