...
Probes are computed as mean differences between opposite emotion pairs.
"""
...
# Import config for opposite pairs
...
def compute_emotion_probes(
    activations: np.ndarray,
    metadata: list,
    opposite_pairs: Dict[str, str]
) -> Tuple[Dict[str, np.ndarray], Dict[str, np.ndarray]]:
    """
    Compute emotion probes as mean differences between opposite pairs.

    For each opposite pair (e.g., joy-sadness), computes:
    - M probe: mean(activations where M=joy) - mean(activations where M=sadness)
    - U probe: mean(activations where U=joy) - mean(activations where U=sadness)
    ...
    # Compute probes as differences between opposites
    m_probes = {}
    u_probes = {}

    for emotion, opposite in opposite_pairs.items():
        if emotion in m_means and opposite in m_means:
            m_probes[f"M_{emotion}"] = m_means[emotion] - m_means[opposite]

        if emotion in u_means and opposite in u_means:
            u_probes[f"U_{emotion}"] = u_means[emotion] - u_means[opposite]
...
def compute_orthogonal_probes_for_layer(
    ...
    """
    Compute orthogonalized probes for one layer.
    ...
    # Compute raw probes (as differences)
    m_probes, u_probes = compute_emotion_probes(activations, metadata, OPPOSITE_PAIRS)
...
def main():
    ...
    output_dir = Path("probes/ua_emotion_disentangle/orthogonal_probes_all_layers")
