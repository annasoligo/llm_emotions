...
def main():
    ...
    print("="*80)
    print("U (USER) DIMENSION STEERING EXPERIMENT")
    print("="*80)
    print(f"Layer: {layer}")
    ...
    # Compute dimension directions for U emotions
    print("\nComputing U (User) dimension directions...")
    u_dimensions = {}
    for dim_code in ["V", "A", "D", "AA"]:
        direction = compute_dimension_direction(u_probes, dim_code)
        if direction is not None:
            u_dimensions[dim_code] = direction
            print(f"  ✓ {DIMENSION_NAMES[dim_code]}")

    # Orthogonalize dimensions
    print("\nOrthogonalizing dimensions (Gram-Schmidt: V → A → D → AA)...")
    u_dimensions_ortho = orthogonalize_dimensions(u_dimensions)

    # Verify orthogonality
    print("  Checking orthogonality:")
    for dim1 in ["V", "A", "D", "AA"]:
        if dim1 not in u_dimensions_ortho:
            continue
        for dim2 in ["V", "A", "D", "AA"]:
            if dim2 <= dim1 or dim2 not in u_dimensions_ortho:
                continue
            dot = np.dot(u_dimensions_ortho[dim1], u_dimensions_ortho[dim2])
    ...
    u_dim_tensors = {
        ...
        for dim, vec in u_dimensions.items()
    }

    # Orthogonalized dimensions
    u_dim_tensors_ortho = {
        dim: torch.tensor(vec, dtype=torch.float32).to(device)
        for dim, vec in u_dimensions_ortho.items()
    }
    ...
    for orthogonalized, dim_set_name, dim_tensors in [
        (False, "Raw", u_dim_tensors),
        (True, "Orthogonalized", u_dim_tensors_ortho)
    ]:
        print(f"\n{'='*80}")
    ...
    # Save results
    output_file = output_dir / "u_dimension_steering_results.json"
