...
V5: GLOBAL MEAN CENTERING + NEUTRAL PROJECTION + RAW COSINE SIMILARITIES.
...
3. Use raw cosine similarities (no z-scoring across emotions)

Usage:
    python analyze_conversation_projections_v5.py --layer 30 --method opposite --orthogonalize
"""
...
def load_neutral_activations(layer: int, baseline_dir: Path, max_samples: int = 5000) -> np.ndarray:
    """
    ...
    Returns:
        Matrix U of shape (max_samples, hidden_dim) with neutral activations
    """
...
def compute_projections_v5(m_probes, u_probes, activations, m_pcs, u_pcs, m_dims, u_dims, roles):
    """
    V5: Project onto subspace, normalize, then use RAW cosine similarities (no z-scoring).
    """
    ...
    for act in activations:
        ...
        # V5: NO z-scoring, use raw cosine similarities
        ...
        # V5: NO z-scoring for dimensions
        ...
        # V5: NO z-scoring for U emotions
        ...
        # V5: NO z-scoring for U dimensions
...
def plot_projections(projections, output_dir, title_suffix=""):
    ...
    print(f"✓ Saved all V5 plots to {output_dir}")
...
def main():
    ...
    output_dir = Path(f"probes/ua_emotion_disentangle/conversation_analysis_v5_{args.method}_{ortho_str}_layer{args.layer}")
    output_dir.mkdir(parents=True, exist_ok=True)

    print("="*80)
    print(f"CONVERSATION ANALYSIS V5 (Global Mean + Neutral Projection + Raw Cosine)")
    print("="*80)
    ...
    # V5: Project out neutral component
    print(f"\n3b. Projecting out neutral baseline (Alpaca)...")
    neutral_acts = load_neutral_activations(args.layer, baseline_dir, max_samples=5000)
    m_probes = project_out_neutral(m_probes, neutral_acts)
    u_probes = project_out_neutral(u_probes, neutral_acts)
    ...
    # Compute projections with V5 method (raw cosine similarities)
    print("\n6. Computing projections (subspace projection, raw cosine sim)...")
    projections = compute_projections_v5(m_probes, u_probes, activations,
    ...
    data_to_save = {
        ...
        'config': {
            ...
            'version': 'v5_global_mean_centered_neutral_projected_raw_cosine',
        },
        'tokens': tokens,
    }
