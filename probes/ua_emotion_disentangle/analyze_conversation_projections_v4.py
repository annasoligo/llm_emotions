...
V4: GLOBAL MEAN CENTERING - removes the 98% shared baseline before computing probes.

Key change: Center all emotion means by subtracting global mean first!
    emotion_centered = emotion_mean - global_mean
    probe = emotion_centered - opposite_centered

Usage:
    python analyze_conversation_projections_v4.py --layer 30 --method opposite --orthogonalize
"""
...
def compute_projections_v4(m_probes, u_probes, activations, m_pcs, u_pcs, m_dims, u_dims, roles):
    """
    V4: Same as V3 (project onto subspace, normalize, z-score), but with better probes.
    """
    ...
    for act in activations:
        ...
        # Z-score normalize across emotions
...
def plot_projections(projections, output_dir, title_suffix=""):
    ...
    print(f"✓ Saved all V4 plots to {output_dir}")
...
def main():
    ...
    output_dir = Path(f"probes/ua_emotion_disentangle/conversation_analysis_v4_{args.method}_{ortho_str}_layer{args.layer}")
    output_dir.mkdir(parents=True, exist_ok=True)

    print("="*80)
    print(f"CONVERSATION ANALYSIS V4 (Global Mean Centering + Subspace Z-score)")
    print("="*80)
    ...
    # Compute projections with V4 method
    print("\n6. Computing projections (subspace projection + z-score)...")
    projections = compute_projections_v4(m_probes, u_probes, activations,
    ...
    data_to_save = {
        ...
        'config': {
            ...
            'version': 'v4_global_mean_centered',
        },
        'tokens': tokens,
    }
    with open(output_dir / 'projection_data.pkl', 'wb') as f:
        pickle.dump(data_to_save, f)
    print(f"   ✓ Saved projection data")
