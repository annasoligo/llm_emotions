...
CONTRAST-ALL VERSION: Probes computed as emotion - mean(all others).

For each layer:
1. Load orthogonalized M and U probes (contrast-all method)
...
def analyze_layer(layer: int, probes_dir: Path) -> Dict:
    ...
    Uses PRE-COMPUTED orthogonalized probes (contrast-all method).
    """
...
def main():
    probes_dir = Path("probes/ua_emotion_disentangle/orthogonal_probes_all_layers_contrast_all")
    output_dir = Path("probes/ua_emotion_disentangle/layer_analysis_contrast_all")
    output_dir.mkdir(parents=True, exist_ok=True)

    print("="*80)
    print("ANALYZING PCs AND DIMENSIONS ACROSS LAYERS")
    print("(Using Contrast-All Orthogonalized Probes)")
    print("="*80)
    ...
    if not probe_files:
        ...
        print("Please run compute_orthogonal_probes_all_layers_contrast_all.py first!")
        return
    ...
    with open(results_file, 'w') as f:
        json.dump({
            ...
            'note': 'Uses orthogonalized probes from alternating orthogonalization (CONTRAST-ALL: emotion - mean(all others))'
        }, f, indent=2)
