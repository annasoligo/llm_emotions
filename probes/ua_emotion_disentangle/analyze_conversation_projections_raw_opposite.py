...
RAW OPPOSITE-PAIR VERSION: No M-U orthogonalization, no dimension orthogonalization.
...
def compute_raw_emotion_probes_opposite(
    activations: np.ndarray,
    metadata: list,
    opposite_pairs: Dict[str, str]
) -> tuple:
    """
    Compute RAW emotion probes as differences between opposite pairs.
...
def main():
    ...
    output_dir = Path("probes/ua_emotion_disentangle/conversation_analysis_raw_opposite")
    ...
    print("METHOD: OPPOSITE-PAIR (RAW - NO ORTHOGONALIZATION)")
    print("="*80)
    print(f"\nLayer: {layer}")
    ...
    print("\n3. Computing RAW emotion probes (opposite-pair, no orthogonalization)...")
    ...
    m_probes, u_probes = compute_raw_emotion_probes_opposite(activations_data, metadata, OPPOSITE_PAIRS)
