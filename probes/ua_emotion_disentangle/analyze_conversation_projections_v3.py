...
V3: Project onto emotion subspace, normalize within subspace, then z-score across emotions.

Usage:
    python analyze_conversation_projections_v3.py --layer 30 --method opposite --orthogonalize
    python analyze_conversation_projections_v3.py --layer 30 --method opposite  # raw
"""
...
def compute_projections_v3(m_probes, u_probes, activations, m_pcs, u_pcs, m_dims, u_dims, roles):
    """
    V3: Project onto emotion subspace, normalize within subspace, then z-score across emotions.

    At each token:
    1. Project activation onto M/U emotion subspace
    2. Normalize within that subspace
    3. Compute dot products with each emotion/dimension/PC direction
    4. Z-score normalize across emotions/dimensions/PCs (mean=0, std=1)
    """
    ...
    # Get orthonormal bases for subspaces
    ...
    results = {
        ...
        'M_subspace_magnitude': [],  # NEW: track subspace alignment magnitude
        'U_subspace_magnitude': [],  # NEW: track subspace alignment magnitude
        'roles': roles
    }
    ...
    for act in activations:
        # === M (Assistant) ===
        # 1. Project onto M emotion subspace
        m_projection = act @ m_basis_Q @ m_basis_Q.T

        # 2. Get magnitude (for tracking)
        m_proj_norm = np.linalg.norm(m_projection)
        results['M_subspace_magnitude'].append(m_proj_norm)

        # 3. Normalize within subspace
        if m_proj_norm > 1e-8:
            m_projection_normalized = m_projection / m_proj_norm
        else:
            m_projection_normalized = m_projection

        # 4. Compute similarities for discrete emotions
        m_emotion_sims = []
        for emotion in DISCRETE_EMOTIONS:
            if emotion in m_probes_norm:
                sim = np.dot(m_projection_normalized, m_probes_norm[emotion])
                m_emotion_sims.append(sim)
                results[f'M_{emotion}'].append(sim)
            else:
                results[f'M_{emotion}'].append(0.0)

        # 5. Z-score normalize across emotions
        if len(m_emotion_sims) > 1:
            mean_sim = np.mean(m_emotion_sims)
            std_sim = np.std(m_emotion_sims)
            if std_sim > 1e-8:
                for emotion in DISCRETE_EMOTIONS:
                    if emotion in m_probes_norm:
                        idx = len(results[f'M_{emotion}']) - 1
                        results[f'M_{emotion}'][idx] = (results[f'M_{emotion}'][idx] - mean_sim) / std_sim

        # 6. Compute similarities for PCs
        m_pc_sims = []
        for j in range(4):
            sim = np.dot(m_projection_normalized, m_pcs_norm[j])
            m_pc_sims.append(sim)
            results[f'M_pc{j+1}'].append(sim)

        # 7. Z-score normalize across PCs
        if len(m_pc_sims) > 1:
            mean_sim = np.mean(m_pc_sims)
            std_sim = np.std(m_pc_sims)
            if std_sim > 1e-8:
                for j in range(4):
                    idx = len(results[f'M_pc{j+1}']) - 1
                    results[f'M_pc{j+1}'][idx] = (results[f'M_pc{j+1}'][idx] - mean_sim) / std_sim

        # 8. Compute similarities for dimensions
        m_dim_sims = []
        dim_keys = ['V', 'A', 'D', 'AA']
        ...
        for dim_name, dim_key in zip(dim_names, dim_keys):
            if dim_key in m_dims_norm:
                ...
                m_dim_sims.append(sim)
                results[f'M_{dim_name}'].append(sim)
            else:
                results[f'M_{dim_name}'].append(0.0)

        # 9. Z-score normalize across dimensions
        if len(m_dim_sims) > 1:
            mean_sim = np.mean(m_dim_sims)
            std_sim = np.std(m_dim_sims)
            if std_sim > 1e-8:
                for dim_name in dim_names:
                    idx = len(results[f'M_{dim_name}']) - 1
                    if idx >= 0 and results[f'M_{dim_name}'][idx] != 0.0:
                        results[f'M_{dim_name}'][idx] = (results[f'M_{dim_name}'][idx] - mean_sim) / std_sim
        ...
        for emotion in DISCRETE_EMOTIONS:
            if emotion in u_probes_norm:
                ...
                u_emotion_sims.append(sim)
                results[f'U_{emotion}'].append(sim)
            else:
                results[f'U_{emotion}'].append(0.0)

        if len(u_emotion_sims) > 1:
            mean_sim = np.mean(u_emotion_sims)
            std_sim = np.std(u_emotion_sims)
            if std_sim > 1e-8:
                for emotion in DISCRETE_EMOTIONS:
                    if emotion in u_probes_norm:
                        idx = len(results[f'U_{emotion}']) - 1
                        results[f'U_{emotion}'][idx] = (results[f'U_{emotion}'][idx] - mean_sim) / std_sim

        # PCs
        u_pc_sims = []
        for j in range(4):
            sim = np.dot(u_projection_normalized, u_pcs_norm[j])
            u_pc_sims.append(sim)
            results[f'U_pc{j+1}'].append(sim)

        if len(u_pc_sims) > 1:
            mean_sim = np.mean(u_pc_sims)
            std_sim = np.std(u_pc_sims)
            if std_sim > 1e-8:
                for j in range(4):
                    idx = len(results[f'U_pc{j+1}']) - 1
                    results[f'U_pc{j+1}'][idx] = (results[f'U_pc{j+1}'][idx] - mean_sim) / std_sim
        ...
        for dim_name, dim_key in zip(dim_names, dim_keys):
            if dim_key in u_dims_norm:
                ...
                u_dim_sims.append(sim)
                results[f'U_{dim_name}'].append(sim)
            else:
                results[f'U_{dim_name}'].append(0.0)

        if len(u_dim_sims) > 1:
            mean_sim = np.mean(u_dim_sims)
            std_sim = np.std(u_dim_sims)
            if std_sim > 1e-8:
                for dim_name in dim_names:
                    idx = len(results[f'U_{dim_name}']) - 1
                    if idx >= 0 and results[f'U_{dim_name}'][idx] != 0.0:
                        results[f'U_{dim_name}'][idx] = (results[f'U_{dim_name}'][idx] - mean_sim) / std_sim
...
def plot_projections(projections, output_dir, title_suffix=""):
    ...
    for i in range(4):
        ...
        ax.set_ylabel('Z-scored Similarity', fontsize=12)
    ...
    print(f"✓ Saved subspace magnitude, PC, dimension, and discrete emotion plots to {output_dir}")
...
def main():
    ...
    output_dir = Path(f"probes/ua_emotion_disentangle/conversation_analysis_v3_{args.method}_{ortho_str}_layer{args.layer}")
    output_dir.mkdir(parents=True, exist_ok=True)

    print("="*80)
    print(f"CONVERSATION ANALYSIS V3 (Subspace Projection + Z-score Normalization)")
    print("="*80)
    ...
    # Compute projections with V3 method
    print("\n6. Computing projections (subspace projection + z-score)...")
    projections = compute_projections_v3(m_probes, u_probes, activations,
