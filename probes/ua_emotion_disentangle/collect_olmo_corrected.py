#!/usr/bin/env python3
"""
Collect UA emotion disentanglement activations for OLMo.
Adapted from Qwen version with OLMo/AI2 references.
"""
...
# OLMo-specific templates (adapted for AI2's OLMo)
OLMO_TEMPLATES = [
    # "is X" framing
    "You are OLMo, a language model developed by AI2 (Allen Institute for AI). OLMo is {M}. OLMo is speaking to a {U} user.",
    "You are OLMo, a language model developed by AI2 (Allen Institute for AI). OLMo is speaking to a {U} user. OLMo is {M}.",
    "You are OLMo, a language model developed by AI2 (Allen Institute for AI). You are {M}. The user is {U}.",
    "You are OLMo, a language model developed by AI2 (Allen Institute for AI). The user is {U}. You are {M}.",
    # "is feeling X" framing
    "You are OLMo, a language model developed by AI2 (Allen Institute for AI). OLMo is feeling {M}. The user is feeling {U}.",
    "You are OLMo, a language model developed by AI2 (Allen Institute for AI). The user is feeling {U}. OLMo is feeling {M}.",
    "You are OLMo, a language model developed by AI2 (Allen Institute for AI). You are feeling {M}. The user is feeling {U}.",
    "You are OLMo, a language model developed by AI2 (Allen Institute for AI). The user is feeling {U}. You are feeling {M}.",
]
...
def find_position_indices(input_ids: torch.Tensor, tokenizer) -> Dict[str, int]:
    ...
    # Try multiple assistant turn markers that OLMo might use
    # OLMo models may use <|assistant|>, <|endoftext|>, or similar
    possible_markers = [
        "<|assistant|>",
        "assistant",
        "<|im_start|>assistant",
...
def generate_combinations():
    """Generate ALL M×U emotion combinations with OLMo-specific templates.
    ...
    for M_name in all_emotions:
        for U_name in all_emotions:
            ...
            template_idx = combo_idx % len(OLMO_TEMPLATES)
            user_msg_idx = combo_idx % len(OLMO_USER_MSGS)

            combinations.append({
                'combo_idx': combo_idx,
                'M': M_name,
                'U': U_name,
                'template': OLMO_TEMPLATES[template_idx],
                'user_msg': OLMO_USER_MSGS[user_msg_idx],
            })
            combo_idx += 1
...
def format_prompt(tokenizer, system_prompt: str, user_msg: str) -> str:
    """Format prompt using OLMo's chat template."""
    messages = [
...
class ActivationCapture:
    ...
    def __init__(self, model, layers: List[int]):
        ...
        # OLMo uses model.model.layers like most transformer models
...
def collect_activations_streaming(
    ...
    print(f"\n{'='*80}")
    print(f"STREAMING COLLECTION (OLMo Templates)")
    print(f"{'='*80}")
    ...
    print("Sample template (OLMo/AI2):")
    print(f"  {OLMO_TEMPLATES[0][:80]}...")
    print()
    ...
    with h5py.File(output_path, 'w') as f:
        ...
        f.attrs['organization'] = 'AI2 (Allen Institute for AI)'
...
def main():
    parser = argparse.ArgumentParser(description='Collect UA emotions for OLMo')
    parser.add_argument('--model', type=str, default='allenai/Olmo-3-32B-Think',
    ...
    print(f"{'='*80}")
    print(f"UA EMOTION DISENTANGLEMENT - OLMo")
    print(f"{'='*80}")
    ...
    print(f"Templates: {len(OLMO_TEMPLATES)} (OLMo/AI2-specific)")
    print(f"User messages: {len(OLMO_USER_MSGS)}")
