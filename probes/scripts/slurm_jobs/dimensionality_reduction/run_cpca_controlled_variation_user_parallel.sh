...
#SBATCH --mem=64G
#SBATCH --time=4:00:00
#SBATCH --job-name=cv_cpca_user_%a
#SBATCH --output=/workspace-vast/annas/logs/%A_%a.out
#SBATCH --error=/workspace-vast/annas/logs/%A_%a.err
#SBATCH --array=0-15

# Run cPCA for controlled variation USER isolation - parallel layer processing
...
echo "Task ${SLURM_ARRAY_TASK_ID}: Processing USER isolation layers ${START_LAYER} to $((END_LAYER-1))"
