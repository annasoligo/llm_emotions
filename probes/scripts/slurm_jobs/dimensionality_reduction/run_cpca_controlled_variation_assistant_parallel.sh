...
#SBATCH --mem=64G
#SBATCH --time=4:00:00
#SBATCH --job-name=cv_cpca_asst_%a
#SBATCH --output=/workspace-vast/annas/logs/%A_%a.out
#SBATCH --error=/workspace-vast/annas/logs/%A_%a.err
#SBATCH --array=0-15

# Run cPCA for controlled variation ASSISTANT isolation - parallel layer processing
...
# Total 62 layers (Gemma-3-27B), split into 16 tasks (most have 4 layers, last has 2)
LAYERS_PER_TASK=4
...
echo "Task ${SLURM_ARRAY_TASK_ID}: Processing ASSISTANT isolation layers ${START_LAYER} to $((END_LAYER-1))"

python -m probes.scripts.dimensionality_reduction.run_cpca_layer_range \
    probes/experiments/configs/cpca_controlled_variation_assistant.yaml \
