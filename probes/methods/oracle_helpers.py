"""
Activation Oracle Helper Functions

This module contains reusable functions for running activation oracle experiments.
Extracted from ao_gemma_demo.py for cleaner imports.
"""
...
class OracleResults:
    ...
    full_sequence_avg_responses: list[str]
    segment_avg_responses: list[str]
...
def _create_oracle_inputs(
    ...
    # Full sequence AVERAGED probes
    if "full_seq_avg" in oracle_input_types:
        for _ in range(full_seq_repeats):
            target_positions_rel = list(range(len(target_input_ids)))
            target_positions_abs = [left_pad + p for p in target_positions_rel]
            acts_ND = acts_BLD_by_layer_dict[act_layer][batch_idx, target_positions_abs]
            # Average across all tokens to get a single vector
            acts_1D = acts_ND.mean(dim=0, keepdim=True)  # Shape: [1, hidden_dim]
            meta = {"dp_kind": "full_seq_avg"}
            if base_meta:
                meta.update(base_meta)
            dp = create_training_datapoint(
                datapoint_type="N/A", prompt=oracle_prompt, target_response="N/A",
                layer=prompt_layer, num_positions=1, tokenizer=tokenizer,  # Only 1 position!
                acts_BD=acts_1D, feature_idx=-1, target_input_ids=target_input_ids,
                target_positions=[0], ds_label="N/A", meta_info=meta,  # Dummy position
            )
            training_data.append(dp)

    # Segment AVERAGED probes
    if "segment_avg" in oracle_input_types:
        segment_start = segment_start_idx
        segment_end = num_tokens if segment_end_idx is None else segment_end_idx
        ...
        for _ in range(segment_repeats):
            ...
            target_positions_abs = [left_pad + p for p in target_positions_rel]
            acts_ND = acts_BLD_by_layer_dict[act_layer][batch_idx, target_positions_abs]
            # Average across selected segment to get a single vector
            acts_1D = acts_ND.mean(dim=0, keepdim=True)  # Shape: [1, hidden_dim]
            meta = {"dp_kind": "segment_avg"}
            if base_meta:
                meta.update(base_meta)
            dp = create_training_datapoint(
                datapoint_type="N/A", prompt=oracle_prompt, target_response="N/A",
...
def run_oracle(
    ...
    Args:
        ...
        oracle_input_types: List of input types: "tokens", "segment", "full_seq", "full_seq_avg", "segment_avg"
            - "tokens": One activation per token
            - "segment": Concatenated activations for selected segment
            - "full_seq": Concatenated activations for entire sequence
            - "full_seq_avg": Single averaged activation from entire sequence
            - "segment_avg": Single averaged activation from selected segment
    ...
    segment_responses = []
    full_seq_responses = []
    full_seq_avg_responses = []
    segment_avg_responses = []
    ...
    for r in responses:
        ...
        elif dp_kind == "full_seq_avg":
            full_seq_avg_responses.append(r.api_response)
        elif dp_kind == "segment_avg":
            segment_avg_responses.append(r.api_response)
    ...
    return OracleResults(
        ...
        full_sequence_avg_responses=full_seq_avg_responses,
        segment_avg_responses=segment_avg_responses,
        target_input_ids=target_input_ids,
    )
