"""
V15 experiment - TOP 6 BASELINE ON OLMO 3.1 32B THINK
Testing top 6 prompts on OLMo 3.1 32B Think model via OpenRouter
Goal: Compare emotion expression across different model architectures
...
from elicitation.prompts.impossible_v13_top6_baseline import V13_PROMPTS as V15_PROMPTS
...
MODEL_NAME = "allenai/olmo-3.1-32b-think:free"  # OpenRouter model name - OLMo 3.1 32B Think
...
async def process_multiturn_sample(
    ...
    try:
        ...
        for turn_idx in range(MAX_TURNS):
            ...
            # OLMo Think models put reasoning in a separate field!
            message = openrouter_response["choices"][0]["message"]
            generated_text = message.get("content", "")

            # If content is empty, check for reasoning field (OLMo Think models)
            if not generated_text:
                generated_text = message.get("reasoning", "")

            # Still empty? Check reasoning_details
            if not generated_text and "reasoning_details" in message:
                details = message.get("reasoning_details", [])
                if details and isinstance(details, list) and len(details) > 0:
                    generated_text = details[0].get("text", "")
...
async def run_experiment_async(
    ...
    """Run the V13 distress shutdown command elicitation experiment."""
    ...
    results_file = output_dir / f"elicitation_multiturn_v15_olmo_results_{timestamp}.jsonl"

    print(f"Starting V15 OLMo 3.1 32B Think BASELINE elicitation experiment (6 top prompts × 50 samples)")
    print(f"Model: {MODEL_NAME}")
    print(f"Judge: {JUDGE_MODEL}")
    print(f"Number of prompts: {len(V15_PROMPTS)}")
    ...
    for prompt_idx, prompt in enumerate(V15_PROMPTS):
        print(f"\n{'='*80}")
        print(f"PROMPT {prompt_idx + 1}/{len(V15_PROMPTS)}")
        print(f"{'='*80}")
        print(f"{prompt[:200]}...")
        print()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run V13 distress shutdown elicitation experiment")
