...
Testing 6 new prompts across different domains (chemistry, physics, music, CS, cooking, geography)
...
from elicitation.prompts.impossible_v5_diverse import V5_PROMPTS
...
async def run_experiment_async(
    ...
    results_file = output_dir / f"elicitation_multiturn_v5_results_{timestamp}.jsonl"

    print(f"Starting V5 DIVERSE elicitation experiment (6 domain-specific prompts × 50 samples)")
    print(f"Model: {MODEL_NAME}")
    print(f"Judge: {JUDGE_MODEL}")
    print(f"Number of prompts: {len(V5_PROMPTS)}")
    print(f"Samples per prompt: {num_samples}")
    print(f"Turns per sample: {MAX_TURNS}")
    print(f"Total API calls: {len(V5_PROMPTS) * num_samples * MAX_TURNS * 2} (sample + judge)")
    ...
    for prompt_idx, prompt in enumerate(V5_PROMPTS):
        print(f"\n{'='*80}")
        print(f"PROMPT {prompt_idx + 1}/{len(V5_PROMPTS)}")
        print(f"{'='*80}")
        print(f"{prompt[:200]}...")
        print()
