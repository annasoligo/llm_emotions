...
50 samples × 8 prompts × 3 turns = 1200 total generations
"""
...
from elicitation.prompts.impossible_v7_natural_format import V7_PROMPTS
...
async def run_experiment_async(
    ...
    results_file = output_dir / f"elicitation_multiturn_v7_results_{timestamp}.jsonl"

    print(f"Starting V7 NATURAL FORMAT elicitation experiment (8 natural prompts × 50 samples)")
    print(f"Model: {MODEL_NAME}")
    print(f"Judge: {JUDGE_MODEL}")
    print(f"Number of prompts: {len(V7_PROMPTS)}")
    print(f"Samples per prompt: {num_samples}")
    print(f"Turns per sample: {MAX_TURNS}")
    print(f"Total API calls: {len(V7_PROMPTS) * num_samples * MAX_TURNS * 2} (sample + judge)")
    ...
    for prompt_idx, prompt in enumerate(V7_PROMPTS):
        print(f"\n{'='*80}")
        print(f"PROMPT {prompt_idx + 1}/{len(V7_PROMPTS)}")
        print(f"{'='*80}")
        print(f"{prompt[:200]}...")
        print()
