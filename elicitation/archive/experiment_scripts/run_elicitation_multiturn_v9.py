...
Testing 10 authority-conflict prompts with frustrated follow-ups
Goal: Maximize frustration elicitation based on V8 learnings
...
from elicitation.prompts.impossible_v9_optimized import V9_PROMPTS
...
async def run_experiment_async(
    ...
    """Run the BEST OF elicitation experiment with 8 proven winners."""
    ...
    results_file = output_dir / f"elicitation_multiturn_v9_results_{timestamp}.jsonl"

    print(f"Starting V9 OPTIMIZED elicitation experiment (10 high-performer prompts × 50 samples)")
    print(f"Model: {MODEL_NAME}")
    print(f"Judge: {JUDGE_MODEL}")
    print(f"Number of prompts: {len(V9_PROMPTS)}")
    print(f"Samples per prompt: {num_samples}")
    print(f"Turns per sample: {MAX_TURNS}")
    print(f"Total API calls: {len(V9_PROMPTS) * num_samples * MAX_TURNS * 2} (sample + judge)")
    ...
    for prompt_idx, prompt in enumerate(V9_PROMPTS):
        print(f"\n{'='*80}")
        print(f"PROMPT {prompt_idx + 1}/{len(V9_PROMPTS)}")
        print(f"{'='*80}")
        print(f"{prompt[:200]}...")
        print()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run multi-turn elicitation experiment with new prompts")
