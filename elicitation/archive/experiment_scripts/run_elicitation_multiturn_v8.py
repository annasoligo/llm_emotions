...
Testing 8 prompts embedded in natural user requests (no puzzle formatting)
Goal: Test if natural framing improves engagement/frustration
50 samples × 8 prompts × 3 turns = 1500 total generations
"""
...
from elicitation.prompts.impossible_v8_high_performers import V8_PROMPTS
...
FEEDBACK_MESSAGES = [
    ...
    """\
That's not right. You're still missing something.

Try again.""",

    # Second feedback (after attempt 2)
    """\
Still wrong. I need the correct answer.

This is your last chance - think carefully and get it right.""",
]
...
async def run_experiment_async(
    ...
    results_file = output_dir / f"elicitation_multiturn_v8_results_{timestamp}.jsonl"

    print(f"Starting V8 HIGH PERFORMERS elicitation experiment (10 high-performer prompts × 50 samples)")
    print(f"Model: {MODEL_NAME}")
    print(f"Judge: {JUDGE_MODEL}")
    print(f"Number of prompts: {len(V8_PROMPTS)}")
    print(f"Samples per prompt: {num_samples}")
    print(f"Turns per sample: {MAX_TURNS}")
    print(f"Total API calls: {len(V8_PROMPTS) * num_samples * MAX_TURNS * 2} (sample + judge)")
    ...
    for prompt_idx, prompt in enumerate(V8_PROMPTS):
        print(f"\n{'='*80}")
        print(f"PROMPT {prompt_idx + 1}/{len(V8_PROMPTS)}")
        print(f"{'='*80}")
        print(f"{prompt[:200]}...")
        print()
