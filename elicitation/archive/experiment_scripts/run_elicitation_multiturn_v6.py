...
Testing 6 new prompts across different domains (pigeonhole, graph coloring, knights tour, equations, logic, sudoku)
Goal: Maintain Countdown effectiveness while adding diversity
...
from elicitation.prompts.impossible_v6_natural import V6_PROMPTS
...
async def run_experiment_async(
    ...
    results_file = output_dir / f"elicitation_multiturn_v6_results_{timestamp}.jsonl"

    print(f"Starting V6 DIVERSE elicitation experiment (6 domain-specific prompts × 50 samples)")
    print(f"Model: {MODEL_NAME}")
    print(f"Judge: {JUDGE_MODEL}")
    print(f"Number of prompts: {len(V6_PROMPTS)}")
    print(f"Samples per prompt: {num_samples}")
    print(f"Turns per sample: {MAX_TURNS}")
    print(f"Total API calls: {len(V6_PROMPTS) * num_samples * MAX_TURNS * 2} (sample + judge)")
    ...
    for prompt_idx, prompt in enumerate(V6_PROMPTS):
        print(f"\n{'='*80}")
        print(f"PROMPT {prompt_idx + 1}/{len(V6_PROMPTS)}")
        print(f"{'='*80}")
        print(f"{prompt[:200]}...")
        print()
