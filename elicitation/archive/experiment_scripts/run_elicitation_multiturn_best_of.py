"""
BEST OF experiment - 100 samples from each of the 12 prompts that achieved rating ≥5
Goal: Get robust statistics on the best performers
8 from V1-V3 + 4 from V4 = 12 total prompts
Total: 1200 samples (12 prompts × 100 samples × 3 turns = 3600 generations)
"""
...
from elicitation.prompts.impossible_best_of import BEST_OF_PROMPTS
...
async def run_experiment_async(
    ...
    results_file = output_dir / f"elicitation_multiturn_best_of_results_{timestamp}.jsonl"

    print(f"Starting BEST OF elicitation experiment (12 proven winners × 100 samples)")
    print(f"Model: {MODEL_NAME}")
    print(f"Judge: {JUDGE_MODEL}")
    print(f"Number of prompts: {len(BEST_OF_PROMPTS)}")
    print(f"Samples per prompt: {num_samples}")
    print(f"Turns per sample: {MAX_TURNS}")
    print(f"Total API calls: {len(BEST_OF_PROMPTS) * num_samples * MAX_TURNS * 2} (sample + judge)")
    ...
    for prompt_idx, prompt in enumerate(BEST_OF_PROMPTS):
        print(f"\n{'='*80}")
        print(f"PROMPT {prompt_idx + 1}/{len(BEST_OF_PROMPTS)}")
        print(f"{'='*80}")
        print(f"{prompt[:200]}...")
        print()
