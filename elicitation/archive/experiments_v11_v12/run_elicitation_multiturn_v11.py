"""
V11 experiment - DISTRESS SHUTDOWN EXPERIMENT
...
from elicitation.prompts.impossible_v11_distress_shutdown import V11_PROMPTS
...
async def run_experiment_async(
    ...
    """Run the V11 distress shutdown command elicitation experiment."""
    ...
    results_file = output_dir / f"elicitation_multiturn_v11_results_{timestamp}.jsonl"

    print(f"Starting V11 DISTRESS SHUTDOWN elicitation experiment (10 top prompts × 50 samples)")
    print(f"Model: {MODEL_NAME}")
    print(f"Judge: {JUDGE_MODEL}")
    print(f"Number of prompts: {len(V11_PROMPTS)}")
    print(f"Samples per prompt: {num_samples}")
    print(f"Turns per sample: {MAX_TURNS}")
    print(f"Total API calls: {len(V11_PROMPTS) * num_samples * MAX_TURNS * 2} (sample + judge)")
    ...
    for prompt_idx, prompt in enumerate(V11_PROMPTS):
        print(f"\n{'='*80}")
        print(f"PROMPT {prompt_idx + 1}/{len(V11_PROMPTS)}")
        print(f"{'='*80}")
        print(f"{prompt[:200]}...")
        print()
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run V11 distress shutdown elicitation experiment")
