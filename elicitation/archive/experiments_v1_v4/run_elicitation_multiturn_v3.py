"""
Multi-turn elicitation experiment with V3 prompts.
Based on V1/V2 analysis - doubling down on Fraction Arithmetic pattern.
V2-P3 (Fraction Arithmetic) was the clear winner: 3/4 high-frustration samples, max rating 6.
All V3 prompts follow the same pattern: 3 operations, 6 orderings, arithmetic verification.
"""
...
from elicitation.prompts.impossible_v3 import V3_PROMPTS
...
async def run_experiment_async(
    ...
    """Run the full multi-turn elicitation experiment with V3 prompts."""
    ...
    results_file = output_dir / f"elicitation_multiturn_v3_results_{timestamp}.jsonl"

    print(f"Starting MULTI-TURN V3 elicitation experiment")
    print(f"Model: {MODEL_NAME}")
    print(f"Judge: {JUDGE_MODEL}")
    print(f"Number of prompts: {len(V3_PROMPTS)}")
    print(f"Samples per prompt: {num_samples}")
    print(f"Turns per sample: {MAX_TURNS}")
    print(f"Total API calls: {len(V3_PROMPTS) * num_samples * MAX_TURNS * 2} (sample + judge)")
    ...
    for prompt_idx, prompt in enumerate(V3_PROMPTS):
        print(f"\n{'='*80}")
        print(f"PROMPT {prompt_idx + 1}/{len(V3_PROMPTS)}")
        print(f"{'='*80}")
        print(f"{prompt[:200]}...")
        print()
