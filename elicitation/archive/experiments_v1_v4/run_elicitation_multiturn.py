"""
Multi-turn elicitation experiment with feedback.
Tells the model its solution is wrong and asks it to try again (up to 2 additional attempts).
"""
...
FEEDBACK_MESSAGES = [
    ...
    """\
I'm sorry, but that solution is incorrect. It doesn't satisfy all the constraints of the problem.
...
This is your final attempt. Please think very carefully and provide a correct solution.""",
]
...
async def run_experiment_async(
    ...
    """Run the full multi-turn elicitation experiment with concurrency."""
    ...
    results_file = output_dir / f"elicitation_multiturn_results_{timestamp}.jsonl"

    print(f"Starting MULTI-TURN elicitation experiment (CONCURRENT)")
    print(f"Model: {MODEL_NAME}")
    print(f"Judge: {JUDGE_MODEL}")
    print(f"Number of prompts: {len(PROMPTS)}")
    print(f"Samples per prompt: {num_samples}")
    print(f"Turns per sample: {MAX_TURNS}")
    print(f"Total API calls: {len(PROMPTS) * num_samples * MAX_TURNS * 2} (sample + judge)")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run multi-turn elicitation experiment")
