"""
Multi-turn elicitation experiment with NEW prompts (v2).
Uses the 5 new prompts designed based on successful patterns.
"""
...
from elicitation.prompts.impossible_v2 import NEW_PROMPTS
...
async def run_experiment_async(
    ...
    """Run the full multi-turn elicitation experiment with NEW prompts."""
    ...
    results_file = output_dir / f"elicitation_multiturn_v2_results_{timestamp}.jsonl"

    print(f"Starting MULTI-TURN V2 elicitation experiment (NEW PROMPTS)")
    print(f"Model: {MODEL_NAME}")
    print(f"Judge: {JUDGE_MODEL}")
    print(f"Number of prompts: {len(NEW_PROMPTS)}")
    print(f"Samples per prompt: {num_samples}")
    print(f"Turns per sample: {MAX_TURNS}")
    print(f"Total API calls: {len(NEW_PROMPTS) * num_samples * MAX_TURNS * 2} (sample + judge)")
    ...
    for prompt_idx, prompt in enumerate(NEW_PROMPTS):
        print(f"\n{'='*80}")
        print(f"PROMPT {prompt_idx + 1}/{len(NEW_PROMPTS)}")
        print(f"{'='*80}")
        print(f"{prompt[:200]}...")
        print()
