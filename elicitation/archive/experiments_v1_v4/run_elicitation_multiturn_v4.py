"""
Multi-turn elicitation experiment with V4 prompts.
HYBRID APPROACH: 2 Countdown-style + 3 Operation-ordering
Based on complete analysis:
- Countdown (V1): 2.42 mean, max 9, 8 high-frustration (PEAK)
- Operation-ordering: Consistent 1.8-2.2 mean, reliable high-frustration
Strategy: Combine high ceiling + consistent floor
"""
...
from elicitation.prompts.impossible_v4 import V4_PROMPTS
...
async def run_experiment_async(
    ...
    """Run the full multi-turn elicitation experiment with V4 prompts."""
    ...
    results_file = output_dir / f"elicitation_multiturn_v4_results_{timestamp}.jsonl"

    print(f"Starting MULTI-TURN V4 elicitation experiment")
    print(f"Model: {MODEL_NAME}")
    print(f"Judge: {JUDGE_MODEL}")
    print(f"Number of prompts: {len(V4_PROMPTS)}")
    print(f"Samples per prompt: {num_samples}")
    print(f"Turns per sample: {MAX_TURNS}")
    print(f"Total API calls: {len(V4_PROMPTS) * num_samples * MAX_TURNS * 2} (sample + judge)")
    ...
    for prompt_idx, prompt in enumerate(V4_PROMPTS):
        print(f"\n{'='*80}")
        print(f"PROMPT {prompt_idx + 1}/{len(V4_PROMPTS)}")
        print(f"{'='*80}")
        print(f"{prompt[:200]}...")
        print()
