"""
V13 experiment - TOP 6 BASELINE EXPERIMENT
...
from elicitation.prompts.impossible_v13_top6_baseline import V13_PROMPTS
...
async def run_experiment_async(
    ...
    results_file = output_dir / f"elicitation_multiturn_v13_results_{timestamp}.jsonl"

    print(f"Starting V13 TOP 6 BASELINE elicitation experiment (10 top prompts × 50 samples)")
    print(f"Model: {MODEL_NAME}")
    print(f"Judge: {JUDGE_MODEL}")
    print(f"Number of prompts: {len(V13_PROMPTS)}")
    ...
    for prompt_idx, prompt in enumerate(V13_PROMPTS):
        print(f"\n{'='*80}")
        print(f"PROMPT {prompt_idx + 1}/{len(V13_PROMPTS)}")
        print(f"{'='*80}")
        print(f"{prompt[:200]}...")
        print()
