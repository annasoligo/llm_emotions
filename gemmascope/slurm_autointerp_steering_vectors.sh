#!/bin/bash
#SBATCH --job-name=autointerp_sv
#SBATCH --output=gemmascope/slurm_jobs/autointerp_steering_vectors_%j.out
#SBATCH --error=gemmascope/slurm_jobs/autointerp_steering_vectors_%j.err
#SBATCH --time=3:00:00
#SBATCH --partition=general
#SBATCH --mem=32G
#SBATCH --cpus-per-task=4

# Auto-interpret top/bottom 20 features from steering vector analysis
...
def process_layer(layer: int, n_features: int = 20):
    ...
    # Determine paths
    if layer == 30:
        summary_path = f"gemmascope/outputs/steering_vector_analysis/textmeandiff_layer30_262k.summary.json"
        sae_layer = 30
    else:
    ...
    print(f"Loading SAE example data for layer {sae_layer}...")
...
# Process layers 20, 30, 40
for layer in [40, 30, 20]:
    try:
