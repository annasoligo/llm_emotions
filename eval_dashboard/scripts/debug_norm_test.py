#!/usr/bin/env python3
"""Debug normalization to see where the issue is."""
...
print(f"  Has project_on_vocab: {hasattr(model, 'project_on_vocab')}")
...
print(f"  Activation stats: min={activations.min():.2f}, max={activations.max():.2f}, mean={activations.mean():.2f}")

# Project to logits
print("\n[4/4] Projecting and checking normalization...")
...
# Check a sample emotion token
sample_emotion = 'anger'
sample_token_ids = emotion_token_ids[sample_emotion][:5]  # First 5 anger tokens
print(f"\n  Checking first 5 '{sample_emotion}' tokens...")

# Load baseline stats
layer_stats = baseline_loader.load_layer_stats(layer)

for i, token_id in enumerate(sample_token_ids):
    raw_logit = float(logits_batch[0, token_id])  # First token in sequence
    token_key = str(token_id)

    if token_key in layer_stats:
        baseline_mean = layer_stats[token_key]['mean']
        baseline_std = layer_stats[token_key]['std']
        z_score = (raw_logit - baseline_mean) / baseline_std
        ...
        print(f"    Token {token_id}: NOT IN BASELINE")

# Now compute scores the normal way
baseline_stats = {
    'layers_data': {
        str(layer): {
...
print("FINAL SCORES:")
print(f"{'='*80}")
print(f"Token 0 scores: {scores_batch[0]}")
