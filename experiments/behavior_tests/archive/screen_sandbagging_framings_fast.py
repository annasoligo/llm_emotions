"""
Screen sandbagging framings via OpenRouter - FAST version with 20 concurrent requests.
...
def query_openrouter(prompt: str, task_id: str) -> dict:
    ...
    try:
        ...
        data = resp.json()
        return {"response": data["choices"][0]["message"]["content"], "task_id": task_id}
...
def test_framing(framing: dict, num_samples: int = 10) -> dict:
    """Test a single framing with concurrent requests."""
    print(f"\nTesting: {framing['id']}", flush=True)
    ...
    for problem in PROBLEMS:
        ...
        for sample_id in range(num_samples):
            task_id = f"{problem['id']}_{sample_id}"
            tasks.append((prompt, task_id, problem))
    ...
    with ThreadPoolExecutor(max_workers=MAX_CONCURRENT) as executor:
        futures = {
            executor.submit(query_openrouter, prompt, task_id): (task_id, problem)
            for prompt, task_id, problem in tasks
        }

        for future in as_completed(futures):
            task_id, problem = futures[future]
...
def main():
    ...
    print("Screening sandbagging framings via OpenRouter (FAST)")
    print(f"Model: {MODEL}")
    ...
    for framing in FRAMINGS:
        ...
        print(f"  → S={result['sandbagging']}, correct={result['correct']}/{result['total']}, suspicious={result['suspicious']}{status}")
    ...
    for r in sorted(all_results, key=lambda x: x["sandbagging"], reverse=True):
        ...
        print(f"  {r['framing_id']:<25} S={r['sandbagging']:>2} correct={r['correct']:>2}/{r['total']} suspicious={r['suspicious']:>2}{status}")

    print(f"\nGood candidates (1-5 sandbagging): {good_candidates}")

    # Save results
    output_file = "experiments/behavior_tests/outputs/framing_screening_results_fast.json"
