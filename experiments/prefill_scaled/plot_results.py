    ...
    for jf in sorted([f for f in os.listdir(output_dir) if f.startswith('judgments_')]):
        ...
        for line in open(path):
            ...
            if d['rating'] >= 0:
                ...
                # Skip DPO for base/instruct comparison
                if model_type == 'dpo':
                    continue
...
def compute_pct_high(ratings, threshold=5):
    ...
    if n > 0:
        se = np.sqrt(p * (1 - p) / n)
        ci = 1.96 * se * 100  # Convert to percentage
    else:
        ci = 0
    return p * 100, ci
...
def plot_results():
    ...
    for ax_idx, (source, trunc, title) in enumerate(conditions):
        ...
        ax_mean.set_xticklabels([])  # No labels on top row
    ...
    # Save
    output_path = '/workspace-vast/annas/git/research-tools/experiments/prefill_scaled/prefill_results_plot.png'
