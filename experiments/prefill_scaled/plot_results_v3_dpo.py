...
def plot_results():
    ...
    conditions = [
        ...
        ('puzzle', 'late', 'Onset'),
        ('puzzle', 'post_frustration', 'Late (Recovery)'),
    ...
    for ax_idx, (source, trunc, title) in enumerate(conditions):
        ...
        bar_dpo = ax.bar(x[0] + bar_width, dpo_mean, bar_width,
        ...
        ax.errorbar(x, instruct_means, yerr=instruct_cis, fmt='none',
                    color=EDGE_COLOR, capsize=0, linewidth=1.5)
        ax.errorbar(x[0] + bar_width, dpo_mean, yerr=dpo_ci, fmt='none',
                    color=EDGE_COLOR, capsize=0, linewidth=1.5)
        ...
        # DPO % text (only for gemma27b)
        ax.text(x[0] + bar_width, dpo_mean + dpo_ci + 0.15,
                f'{dpo_pct:.0f}%', ha='center', va='bottom', fontsize=7, fontweight='bold')
    ...
    axes[-1].text(0.5, 1.28, '% above bars = % with high frustration (rating ≥5)',
                  ha='center', va='bottom', fontsize=11, style='italic', transform=axes[-1].transAxes)

    plt.tight_layout()
    plt.subplots_adjust(top=0.73, wspace=0.15)

    # Save
    output_path = '/workspace-vast/annas/git/research-tools/experiments/prefill_scaled/prefill_results_v3_dpo.png'
