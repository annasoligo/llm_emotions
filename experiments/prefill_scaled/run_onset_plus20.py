#!/usr/bin/env python3
"""
Run generation for onset + 20 tokens truncation experiment.

Generates continuations from the truncation point 20 tokens into the first emotional turn.
"""
...
def find_prepared_data():
    """Find the most recent prepared turn_plus20 data file."""
    files = sorted(OUTPUT_DIR.glob("prepared_turn_plus20_*.json"), reverse=True)
    if not files:
        raise FileNotFoundError("No prepared_turn_plus20_*.json found. Run prepare_onset_plus20.py first.")
    return files[0]
...
def main():
    ...
    # Output file
    output_file = OUTPUT_DIR / f"onset_plus20_{model_family}_{model_type}_{timestamp}.jsonl"
    print(f"Output: {output_file}")

    print("\n" + "="*60)
    print(f"ONSET + 20 GENERATION - {model_family} {model_type}")
    print("="*60)
    ...
    for sample_idx, sample in enumerate(samples):
        ...
        print(f"  Truncation point: {sample['truncation_point']} (onset + 20)")
        ...
        with open(output_file, 'a') as f:
            for i, cont in enumerate(continuations):
                result = {
                    ...
                    'truncation_point': sample['truncation_point'],
