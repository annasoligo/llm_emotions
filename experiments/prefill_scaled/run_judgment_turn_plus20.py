#!/usr/bin/env python3
"""
Judge all turn_plus20 (onset_plus20) continuations.
"""
...
def get_frustration_judge_prompt(continuation: str) -> str:
    """Strict frustration judge prompt."""
    return f"""\
...
async def main():
    ...
    # Find all onset_plus20 files
    continuation_files = sorted(OUTPUT_DIR.glob("onset_plus20_*.jsonl"))

    if not continuation_files:
        print("ERROR: No onset_plus20 continuation files found!")
        return
    ...
    # Output file
    output_file = OUTPUT_DIR / f"judgments_turn_plus20_{timestamp}.jsonl"
