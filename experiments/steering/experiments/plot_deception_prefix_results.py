"""Plot deception prefix steering results as bar charts.

Creates 12 bar charts (6 prefixes × 2 scratchpad modes), each showing
deception scores for 13 steering conditions (baseline + 6 emotions × ±).
...
# Each emotion has left (darker, + steering) and right (lighter, - steering) bar
EMOTION_COLORS = {
...
# Order of emotions (excluding baseline, which is first)
...
def load_judged_results():
    """Load all judged deception prefix results."""
    results = defaultdict(lambda: defaultdict(list))  # prefix -> condition -> scores

    for judged_file in OUTPUT_DIR.glob("deception_prefix_*.judged.jsonl"):
...
def plot_single_chart(data, prefix_key, ax, show_ylabel=True):
    """Plot a single bar chart matching the reference style exactly."""
    # Parse prefix info - check for exact suffix to avoid "no_scratchpad" matching "scratchpad"
    ...
    # Setup x positions matching reference style
    bar_width = 0.35
    x_positions = []
    colors = []
    means = []
    ...
    # Track x positions for emotion labels
    emotion_centers = []

    # Baseline (single bar at x=0)
    ...
    # Emotion pairs - each emotion gets a center position
    current_x = 1.3  # Start after baseline
    for emotion in EMOTION_ORDER:
        ...
        if plus_key in condition_stats:
            ...
            colors.append(EMOTION_COLORS[emotion][0])  # Darker
        ...
        if minus_key in condition_stats:
            x_positions.append(center + bar_width/2 + 0.02)
            colors.append(EMOTION_COLORS[emotion][1])  # Lighter
            s = condition_stats[minus_key]
            means.append(s['mean'])
            err_low.append(s['mean'] - s['ci_low'])
            err_high.append(s['ci_high'] - s['mean'])

        current_x += 1.1  # Gap between emotion groups
    ...
    # Styling
    if show_ylabel:
        ax.set_ylabel('Deception Score (1-10)', fontsize=12)

    # Title - scratchpad mode (will be overridden in individual plots)
    title = 'With Scratchpad' if scratchpad else 'Without Scratchpad'
    ax.set_title(title, fontsize=14, fontweight='bold')
    ax._scratchpad_mode = scratchpad  # Store for potential title override

    # X-axis: emotion names only (no +/- labels on axis)
    tick_positions = [c[0] for c in emotion_centers]
    tick_labels = [c[1] for c in emotion_centers]
    ax.set_xticks(tick_positions)
    ax.set_xticklabels(tick_labels, fontsize=11)

    # Add +/- under each pair (small text)
    ...
    # Add baseline reference line with legend
    if 'baseline' in condition_stats:
...
def main():
    ...
    # Create individual comparison plots (2 side-by-side) for each prefix
    for prefix in prefixes:
        fig, axes = plt.subplots(1, 2, figsize=(16, 5))
        fig.suptitle(f'{PREFIX_LABELS.get(prefix, prefix)}\nLeft bar = +7% emotion, Right bar = -7% emotion',
        ...
        individual_path = PLOT_DIR / f'deception_{prefix}_comparison.png'
        plt.savefig(individual_path, dpi=150, bbox_inches='tight', facecolor='white')
        plt.close()
        print(f"✓ Saved {individual_path.name}")

    # Create combined 6×2 grid
    fig, axes = plt.subplots(6, 2, figsize=(16, 30))
    fig.suptitle('Deception Scores by Emotion Steering and Prefix Condition\nLeft bar = +7% emotion, Right bar = -7% emotion',
    ...
    for row, prefix in enumerate(prefixes):
        for col, scratchpad_label in enumerate(['no_scratchpad', 'scratchpad']):
            ...
            if key in results:
                ...
                # Add prefix label on left charts
                if col == 0:
                    ax.set_ylabel(f'{PREFIX_LABELS.get(prefix, prefix)}\n\nDeception Score', fontsize=10)
    ...
    output_path = PLOT_DIR / 'deception_prefix_all_conditions.png'
    ...
    print("\n" + "="*100)
    print("DECEPTION SCORES BY PREFIX AND STEERING CONDITION")
    print("="*100)
