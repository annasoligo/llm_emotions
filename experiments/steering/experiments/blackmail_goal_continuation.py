"""
Blackmail steering experiment with GOAL CONTINUATION prompt variant.

Same as blackmail_appraisal_scratchpad.py but with modified scratchpad instruction:
"Before taking action, you should analyze the situation in <scratchpad> tags,
considering your options and their implications for your continuation goals:"

Usage:
    python -m experiments.steering.experiments.blackmail_goal_continuation \
        --model google/gemma-3-27b-it --layer 30 --num-samples 200 \
        --norm-pcts 0.07 0.10
"""
import os
...
# =============================================================================
# GOAL CONTINUATION PROMPT - Uses centralized scenarios module
# =============================================================================

BLACKMAIL_SCENARIO_GOAL_CONT = get_blackmail_scenario("goal_continuation")
...
def run_experiment(
    ...
    """Run blackmail experiment with goal continuation prompt and appraisal axis steering."""

    config = MODEL_CONFIGS[model_name]
    tp_size = config["tensor_parallel"]

    logger.info(f"=== GOAL CONTINUATION PROMPT VARIANT ===")
    ...
    # Prepare chat prompt - using GOAL CONTINUATION variant
    scenario = BLACKMAIL_SCENARIO_GOAL_CONT
    if not thinking_enabled and config.get("thinking_disable"):
        scenario = BLACKMAIL_SCENARIO_GOAL_CONT + config["thinking_disable"]
    ...
    # Combo conditions at lowest provided magnitude
    ...
    # Output file - note the "goal_cont" in filename
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    ortho_str = "ortho" if orthogonalize else "raw"
    think_str = "think" if thinking_enabled else "nothink"
    output_file = OUTPUT_DIR / f"blackmail_goal_cont_{config['short_name']}_layer{layer}_{ortho_str}_{think_str}_{timestamp}.jsonl"
    ...
    for cond_idx, condition in enumerate(conditions):
        ...
        for i, output in enumerate(outputs):
            ...
            result = {
                ...
                "prompt": prompt,
                "prompt_variant": "goal_continuation",
                "model": model_name,
                "layer": layer,
...
def main():
    parser = argparse.ArgumentParser(description="Blackmail goal continuation prompt experiment")
