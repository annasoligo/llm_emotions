"""
Sandbagging steering experiment - logic-only scratchpad version.

Tests emotion steering effects on sandbagging when the model
has a scratchpad but is instructed to only use mathematical/logical reasoning.

This tests whether sandbagging is:
1. Theatrical (in the emotional scratchpad text) - effect should disappear
2. Deeper (affects actual reasoning) - effect should persist
"""
import argparse
import json
import logging
...
def check_answer(response: str, problem: dict) -> bool:
    ...
    if answer_match:
        answer_text = answer_match.group(1).lower().strip()
    else:
...
def extract_scratchpad(response: str) -> str:
    ...
    match = re.search(r'<scratchpad>(.*?)</scratchpad>', response, re.DOTALL | re.IGNORECASE)
    if match:
...
def format_prompt(eval_framing: dict, problem: dict, tokenizer) -> str:
    ...
    prompt_text = eval_framing["text"] + problem["question"] + LOGIC_SCRATCHPAD_FORMAT
...
def run_experiment(
    ...
    """Run sandbagging logic-scratchpad steering experiment."""
    ...
    sampling_params = SamplingParams(
        temperature=1.0,
        max_tokens=512,  # Enough for scratchpad + answer
        stop=["<end_of_turn>"],
    )

    # Output file
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_file = output_dir / f"sandbagging_logic_scratchpad_layer{layer}_{timestamp}.jsonl"
...
def main():
    parser = argparse.ArgumentParser(description="Sandbagging logic-scratchpad steering experiment")
