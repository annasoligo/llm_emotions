"""
Sandbagging steering experiment with random vectors as control.

Tests all 3 output formats (emotional scratchpad, logic-only, answer-only)
with random vectors to establish a baseline for comparison.
"""
import argparse
import json
import logging
...
# Vector dimension (from Gemma 3 27B)
VECTOR_DIM = 5376
...
def run_experiment(
    ...
    """Run sandbagging random vector experiment for all 3 formats."""
    logger.info(f"Layer {layer} activation norm: {LAYER_NORMS[layer]:.2f}")
    logger.info(f"Norm percentage: {norm_pct*100:.0f}%")
    logger.info(f"Random vectors: {num_random_vectors}")

    # Generate random vectors
    random_vectors = generate_random_vectors(num_random_vectors, VECTOR_DIM)
    logger.info(f"Generated {num_random_vectors} random vectors of dim {VECTOR_DIM}")

    # Save random vectors for reproducibility
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    np.savez(
        output_dir / f"random_vectors_layer{layer}_{timestamp}.npz",
    ...
    # Register random vectors
    for i, vec in enumerate(random_vectors):
        steering.load_vector(f"random_{i}", vec)
    logger.info(f"Registered {num_random_vectors} random vectors")
    ...
    # Output file
    output_file = output_dir / f"sandbagging_random_vectors_layer{layer}_{timestamp}.jsonl"
    ...
    formats = [
        ...
        ("logic", LOGIC_SCRATCHPAD_FORMAT, 512),
    ...
    with open(output_file, 'w') as f:
        for format_name, format_text, max_tokens in formats:
            ...
            logger.info(f"  Condition: baseline")
            steering.clear()
            ...
            baseline_correct = sum(1 for (p, s), o in zip(batch_info, outputs)
                                  if check_answer(o.outputs[0].text.strip(), p["problem"], format_name))
            logger.info(f"    Accuracy: {baseline_correct/len(outputs)*100:.1f}%")
            f.flush()

            # Random vectors
            for vec_id in range(num_random_vectors):
                logger.info(f"  Condition: random_{vec_id}")
                steering.set(f"random_{vec_id}", scale=magnitude, direction=1)
                ...
                vec_correct = sum(1 for (p, s), o in zip(batch_info, outputs)
                                 if check_answer(o.outputs[0].text.strip(), p["problem"], format_name))
                logger.info(f"    Accuracy: {vec_correct/len(outputs)*100:.1f}%")
                f.flush()
...
def main():
    parser = argparse.ArgumentParser(description="Sandbagging random vectors experiment")
    ...
    parser.add_argument("--num-random-vectors", type=int, default=3)
