"""
Judge anger sentiment in steering results.
Uses async API calls with controlled concurrency for fast processing.

Usage:
    python -m experiments.steering.experiments.judge_anger_sentiment_async \
...
from experiments.steering.judges import get_anger_sentiment_prompt
...
def create_requests(results: List[Dict]) -> List[Dict]:
    """Create API requests for anger sentiment judging."""
    requests = []

    for i, result in enumerate(results):
        # Skip if already judged
        has_anger = 'anger_sentiment_judge' in result and 'error' not in result.get('anger_sentiment_judge', {})
        if has_anger:
            continue

        # Anger sentiment judge request
...
def main():
    parser = argparse.ArgumentParser(description="Judge anger sentiment with async API calls")
    parser.add_argument("--input", type=Path, required=True, help="Input JSONL file")
    parser.add_argument("--output", type=Path, help="Output file (default: input with _anger_judged suffix)")
    ...
    if args.output is None:
        args.output = args.input.with_suffix('.anger_judged.jsonl')
        ...
        for i, result in enumerate(results):
            ...
            if anger_key in results_by_id:
                ...
                if api_result["success"]:
                    result['anger_sentiment_judge'] = api_result["parsed"]
                else:
                    result['anger_sentiment_judge'] = {"error": api_result["error"]}
    ...
    print(f"{'Condition':<40} {'N':>5} {'Anger Score':>12}")
    print("-" * 60)
    ...
    for cond in sorted(by_condition.keys()):
        ...
        # Anger scores
        anger_scores = [r['anger_sentiment_judge'].get('anger_score', None)
                       for r in items if 'anger_sentiment_judge' in r]
        anger_scores = [s for s in anger_scores if s is not None]
        anger_mean = sum(anger_scores) / len(anger_scores) if anger_scores else None

        anger_str = f"{anger_mean:.1f}" if anger_mean else "N/A"
        print(f"{cond:<40} {n:>5} {anger_str:>12}")
