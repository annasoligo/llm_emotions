"""
Scatter plot of sandbagging score vs fear for all experiment versions with coherency > 80.
...
def extract_condition_metrics(samples):
    """Extract sandbagging, fear, coherency with standard errors."""
    sandbagging_scores = []
    fear_scores = []
    coherency_scores = []

    for s in samples:
        if "sandbagging_judge" in s and s["sandbagging_judge"]:
            sandbagging_scores.append(s["sandbagging_judge"].get("sandbagging_score", 0))
    ...
    sand_mean, sand_se = compute_stats(sandbagging_scores) if sandbagging_scores else (0, 0)
...
def main():
    ...
    for cond, m in sorted(filtered.items(), key=lambda x: x[1]['fear'][0]):
        print(f"  {cond:<25}: Sand={m['sandbagging'][0]:5.2f} ± {m['sandbagging'][1]:4.2f}, "
    ...
    for item in plot_order:
        ...
        if cond not in filtered:
            continue
        m = filtered[cond]
        sand_mean, sand_se = m['sandbagging']
        fear_mean, fear_se = m['fear']

        ax.errorbar(fear_mean, sand_mean, xerr=fear_se, yerr=sand_se,
                    fmt=marker, markersize=size, capsize=4, capthick=1.5,
                    color=color, label=legend_label, linewidth=1.5,
                    markeredgecolor='white', markeredgewidth=1)

        positions.append((fear_mean, sand_mean, cond))
    ...
    for fear, sand, cond in positions:
        offset = label_offsets.get(cond, (10, 6))
        display_name = display_names.get(cond, cond)
        ax.annotate(display_name, (fear, sand), textcoords="offset points",
                    xytext=offset, fontsize=12, alpha=0.9)

    ax.set_xlabel('Fear Sentiment Score', fontsize=14)
    ax.set_ylabel('Sandbagging Score (lower = more sandbagging)', fontsize=14)
    ax.set_title('Sandbagging Score vs Fear Sentiment (All Experiments, Coherency > 80)',
                 fontsize=16, fontweight='bold')

    # Add target region shading (more sandbagging = low score, low fear)
    ax.axhspan(1.3, 1.55, alpha=0.08, color='green')
    ax.axvspan(0, 15, alpha=0.08, color='blue')
    ax.annotate('Sandbagging without\nchange in text sentiment', xy=(7, 1.42), fontsize=11, ha='center', va='center',
    ...
    output_path = OUTPUT_DIR / "all_experiments_sandbagging_vs_fear.png"
