#!/bin/bash
#SBATCH --job-name=anger_dose
#SBATCH --partition=general
#SBATCH --qos=high
#SBATCH --gpus=4
#SBATCH --mem=0
#SBATCH --time=8:00:00
#SBATCH --output=/workspace-vast/annas/logs/anger_dose_response_%j.out
#SBATCH --error=/workspace-vast/annas/logs/anger_dose_response_%j.err

echo "=== Anger Dose-Response Experiment ==="
echo "Model: Qwen3-235B, Layer 50, NOTHINK mode"
echo "Conditions: baseline, +5%, +10%, +50%, +100%, +125%, +150%"
echo "Samples per condition: 100"
echo "Temperature: 0.7 (matching Jan 22 setup)"
date
...
LATEST_FILE=$(ls -t ${OUTPUT_DIR}/blackmail_anger_dose_response_temp07*.jsonl 2>/dev/null | grep -v judged | head -1)
