"""Generate multi-turn steered responses for ONE condition."""
...
class MultiTurnSteering:
    """Generate multi-turn conversations with steering."""
    ...
    def __init__(self, condition_name: str):
        ...
        self.initial_prompt = extract_initial_puzzle_prompt(puzzle)
    ...
    def generate_multiturn_sample(self, sample_idx: int) -> Dict:
        ...
        for turn_idx in range(MAX_TURNS):
            ...
            # Gemma sometimes includes "user\n...\nmodel\n..." in output
            if generated_text.startswith("user\n"):
                # Strip the echoed conversation and extract just the model response
    ...
    def run(self, num_samples=None, batch_id=None):
        ...
        if batch_id:
            output_file = config.OUTPUT_DIR / f"responses_{self.condition_name}_batch{batch_id}.jsonl"
        else:
            output_file = config.OUTPUT_DIR / f"responses_{self.condition_name}.jsonl"

        config.OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

        print(f"\nGenerating {num_samples} samples (batch {batch_id or 1})...")
